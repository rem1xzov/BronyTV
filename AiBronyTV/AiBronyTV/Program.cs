...
using (var scope = app.Services.CreateScope())
{
    ...
    if (!string.IsNullOrWhiteSpace(pgHost) && db.Database.ProviderName?.Contains("Npgsql", StringComparison.OrdinalIgnoreCase) == true)
    {
        ...
        await db.Database.ExecuteSqlRawAsync(
            "CREATE TABLE IF NOT EXISTS ai.\"GroupChats\" " +
        ...
        await db.Database.ExecuteSqlRawAsync(
            "CREATE TABLE IF NOT EXISTS ai.\"GroupChatParticipants\" " +
            "(\"GroupChatId\" integer NOT NULL, " +
            "\"CharacterId\" character varying(32) NOT NULL, " +
            "\"AddedAt\" timestamp with time zone NOT NULL, " +
            "CONSTRAINT \"PK_GroupChatParticipants\" PRIMARY KEY (\"GroupChatId\", \"CharacterId\"));");
        await db.Database.ExecuteSqlRawAsync(
            "CREATE TABLE IF NOT EXISTS ai.\"GroupChatMessages\" " +
            "(\"Id\" integer GENERATED BY DEFAULT AS IDENTITY, " +
            "\"GroupChatId\" integer NOT NULL, " +
            "\"SenderType\" character varying(16) NOT NULL, " +
            "\"SenderCharacterId\" character varying(32), " +
            "\"Content\" text NOT NULL, " +
            "\"CreatedAt\" timestamp with time zone NOT NULL, " +
            "CONSTRAINT \"PK_GroupChatMessages\" PRIMARY KEY (\"Id\"));");
    }
}
...
// Групповые чаты (публичный и админский разделы).
app.MapGroupChats("/api/group-chats", isAdminChat: false);
app.MapGroupChats("/api/admin/group-chats", isAdminChat: true);
